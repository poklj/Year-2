		...
		while (!glfwWindowShouldClose(window)) {
			...
			for (int x = 0; x <= 0; x++) {
				for (int z = 0; z <= 0; z++) {
					// for (int x = -2; x <= 2; x++) {
					// for (int z = -2; z <= 2; z++) {
					modelMatrix.translation(x * 2.0f+diff, 0+diff, z * 2.0f+diff).rotateY(angle * (float) Math.toRadians(90));
					//modelMatrix.translation(x * 2.0f, 0, z * 2.0f).rotateX(angle * (float) Math.toRadians(90));
